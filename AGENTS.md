...
- **Blog Posts:** Markdown files located in `src/content/blog/`. Contains YAML frontmatter (`title`, `excerpt`, `date`, `category`, `imageUrl`, `draft`).
- **Resources:** Markdown files located in `src/content/resources/`. Contains YAML frontmatter (`title`, `description`, `date`, `tags`, `icon`, `resource_url`, `featured`, `draft`).
